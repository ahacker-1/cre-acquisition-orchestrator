...
| Scanned PDF extracts no fields | OCR ran but the scan is unreadable, table-heavy, or does not include supported headline fields | Review the OCR warnings and source document manually. The bridge will not guess deal inputs when confidence is low or supported fields are absent. |

...
| `POST` | `/api/run/start` | Start an offline simulation or live Codex run. | `StartRunRequest` with deal/scenario/workflow/runtime fields. | `202` with `runId`, `status`, `mode`, `speed`, `workflowId`, `runtimeProvider`, `pid`, `startedAt`. |
...
| `POST` | `/api/workflows/:workflowId/launch` | Launch a selected workflow. | `WorkflowLaunchRequest` with deal, scenario, speed, mode, runtime provider, Codex options, and source-backed input enforcement. | `WorkflowLaunchResponse` plus `workflow`, `deal`, `readiness`, and `inputSnapshot`. |

...
* **Open evaluation — live-agent accuracy + narrative-risk proof:** added `npm run eval:offline` (no-API extraction + simulation layers) and hardened the live (Codex) layer from an honest baseline. A required machine-parseable agent `## Metrics` block (amortizing-basis DSCR) + a threshold-driven IC verdict rule, plus fixes to the eval's live-workpaper extractor (source EGI from the opex analyst; horizontal-only value↔label matching so a value never binds across a line break; reject pro-forma/stabilized/exit/interest-only variants; parse `DSCR (amortizing): X`), lifted live determinable-financial accuracy and IC verdict to target. The full 8-deal benchmark (offline on all 8; live on a representative 6-deal subset) now proves the live agents genuinely detect **narrative risks** — tenant concentration, insurance understatement, missing Phase I — that the deterministic fixture is structurally blind to: **live narrative red-flag recall 100%, IC verdict 100% exact (n=6), determinable financial 96%**. The remaining honest soft spot is model-dependent returns (IRR / equity multiple) at ~50%. Nothing was tuned to flatter; ground truth unchanged. See [eval/results/TRUST-REPORT.md](eval/results/TRUST-REPORT.md).

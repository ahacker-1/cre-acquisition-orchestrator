# Multifamily Benchmarks & Market Standards
...
- **Rent roll analyst** (validate rent levels): Read the Rent Benchmarks section to validate whether in-place rents and market rent assumptions are reasonable for the property's class, unit type, and market tier. Also read the Loss-to-Lease and Ancillary Income sections to assess value-add potential.
- **OpEx analyst** (validate expense ratios): Read the Operating Expense Benchmarks section -- by property class, by region, and by unit count -- to identify whether the property's expenses are within normal ranges. Flag any line item that falls significantly outside the benchmark range.
- **Market study agent** (compare to market): Read the Cap Rate Ranges, Per-Unit Valuation, Occupancy Standards, and Performance KPIs sections to compare the subject property against market standards. Use these benchmarks to assess relative value and competitive positioning.
- **Financial model builder** (sanity-check projections): After building the financial model, read the Investment Thresholds and Red Flags sections to sanity-check all projections. Any projected metric that falls outside benchmark ranges should be flagged for review.

### What to Cross-Reference

- **Benchmarks against property class, region, and unit count from `deal.json`**: Look up the correct benchmark range by matching the property's class (A/B/C), region (Sun Belt, Northeast, Midwest, West Coast, Mountain West), and unit count. Using national benchmarks for a local market produces misleading comparisons.
- **Actual rents against benchmark ranges**: Compare the property's in-place rents (by unit type) against the rent benchmarks for the appropriate class and market tier. Rents above the top of the range may indicate overpricing risk; rents well below may signal value-add opportunity or property condition issues.
- **Actual expenses against per-unit benchmarks**: Compare each expense line item (insurance, maintenance, management fee, payroll, turnover, utilities) against the per-unit benchmarks for the property's class. Use the regional adjustments to account for geographic cost differences.

### How to Apply

- **Look up the benchmark range**: Identify the property's class (A/B/C), region, and unit count from deal.json. Find the corresponding benchmark ranges in the appropriate tables. These ranges define the "normal" band for that property profile.
- **Compare actual metrics**: For each key metric (rent per unit, rent per SF, OpEx per unit, OpEx ratio, cap rate, occupancy, turnover rate), compare the property's actual figure against the benchmark range.
- **Flag outliers**: Any metric that falls outside the benchmark range is a data point for risk scoring. Metrics above the range may indicate inefficiency or overpricing. Metrics below the range may indicate underperformance or value-add opportunity. Document the deviation and include it in the agent's findings.

### Common Mistakes

- **Using national benchmarks for a local market**: National averages mask enormous regional variation. A $7,000/unit/year OpEx figure is excellent in the Northeast but high for the Sun Belt. Always use the regional and class-specific benchmarks, not the national aggregate.
- **Not adjusting for building age and vintage**: Older buildings (pre-1980) have structurally higher maintenance costs, insurance premiums, and capital reserve requirements. Comparing a 1960s Class C property against new-construction Class A benchmarks produces meaningless results. Consider the property's age when selecting the appropriate benchmark tier.
- **Comparing Class A benchmarks to a Class C property**: Each class has distinct operating characteristics. Class A properties have higher rents but also higher payroll, amenity maintenance, and marketing costs. Class C properties have lower rents but higher turnover costs, maintenance, and bad debt. Always match the benchmark class to the subject property's actual class.

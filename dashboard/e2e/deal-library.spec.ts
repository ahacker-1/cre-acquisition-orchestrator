...
test('reopens a saved draft deal in the edit wizard from the deal library', async ({ page, request }) => {
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(wizard.getByTestId('deal-wizard-save-draft')).toBeFocused()
...
test('runs quick deal screen workflow to completion with skipped phases and package view', async ({ page, request }) => {
  ...
  const firstWorkpaperLink = packageView.locator('[data-testid^="package-workpaper-open-"]').first()
  await expect(firstWorkpaperLink).toBeVisible()
  await expect(firstWorkpaperLink).toHaveAttribute('href', /\/api\/deals\/[^/]+\/artifacts\?path=/)
